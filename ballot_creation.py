...
def get_presidential_election_id() -> int | None:
    try:
        with DatabaseManager() as db:
            db.execute_query(
                "SELECT id FROM elections WHERE position = 'president' AND phase = 'voting' ORDER BY id DESC LIMIT 1"
            )
            row = db.fetch_one()
...
def get_mp_election_id() -> int | None:
    try:
        with DatabaseManager() as db:
            db.execute_query(
                "SELECT id FROM elections WHERE position = 'mp' AND phase = 'voting' ORDER BY id DESC LIMIT 1"
            )
            row = db.fetch_one()
            return row[0] if row else None
    except Exception as e:
...
def display_mp(voter_id: str, election_id: int | None = None) -> dict[str, str]:
    try:
        ...
        with DatabaseManager() as db:
            db.execute_query(
                'SELECT constituency_id, polling_station_id FROM voterinfo WHERE voter_id = %s', (voter_id,)
            )
            voter = db.fetch_one()
            if not voter:
