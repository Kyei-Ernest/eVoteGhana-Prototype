    ...
    try:
        ...
        if has_mop:
            ...
            for (const_name,) in old_constituencies:
                if const_name:
                    cursor.execute(
                        'INSERT IGNORE INTO constituencies(name, region_id) VALUES (%s, %s)',
            ...
            cursor.execute(
                ...
                "AND column_name NOT IN ('id', 'constituency')",
                (dbname,),
            )
            ...
            for party_col in party_columns:
                ...
                cursor.execute(
                    'SELECT presidential_candidate_name FROM presidents WHERE political_party = %s', (party_name,)
                )
                pres = cursor.fetchone()
                if pres:
                    cursor.execute(
                        "INSERT INTO elections(title, position, phase) VALUES (%s, 'president', 'closed')",
                ...
                for row in mop_rows:
                    if col_index < len(row) and row[col_index]:
                        ...
                        if const_name:
                            ...
                            cursor.execute(
                                "INSERT INTO elections(title, position, phase) VALUES (%s, 'mp', 'closed')",
                            ...
                            cursor.execute(
                                """INSERT INTO candidates(name, party_id, constituency_id, election_id)
                                              VALUES (%s, %s, %s, %s)""",
                                (row[col_index], party_id, const_id, eid),
                            )
                            mp_count += 1
        ...
        if has_constituency_col:
            ...
            for col, ddl in [
                ...
                cursor.execute(
                    ...
                    "AND table_name = 'voterinfo' AND column_name = %s",
                    (dbname, col),
                )
                if cursor.fetchone()[0] == 0:
                    cursor.execute(f'ALTER TABLE voterinfo ADD COLUMN {col} {ddl} NULL')
            ...
            for (const_name,) in cursor.fetchall():
                ...
                if const_row:
                    cid = const_row[0]
                    cursor.execute(
                        'UPDATE voterinfo SET constituency_id = %s WHERE constituency = %s', (cid, const_name)
                    )
        ...
        if not cursor.fetchone():
            cursor.execute(
                ...
                ('admin', bcrypt.hashpw(b'admin123', bcrypt.gensalt()).decode('utf-8')),
            )
