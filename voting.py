...
def vote_mp() -> None:
    ...
    try:
        with DatabaseManager() as db:
            ...
            db.execute_query("SELECT voted, polling_station_id, personal_id FROM voterinfo WHERE voter_id = %s", (voters_id,))
            result = db.fetch_one()
            ...
            if not voted_already:
                ...
                if pw_result:
                    ...
                    if verify_password(stored_pwd, password):
                        ...
                        db.execute_query("INSERT INTO votes(voter_id, candidate_id, election_id, polling_station_id, hmac_hash, ballot_paper_id) VALUES (%s, %s, %s, %s, %s, %s)", (voters_id, candidate_id, mp_election_id, polling_station_id, hmac_hash, ballot_id))
                        db.execute_query("UPDATE voterinfo SET mp_vote = %s WHERE voter_id = %s", (candidate_id, voters_id))
...
def vote_president(voters_id: str) -> None:
    try:
        ...
        with DatabaseManager() as db:
            ...
            db.execute_query("INSERT INTO votes(voter_id, candidate_id, election_id, polling_station_id, hmac_hash, ballot_paper_id) VALUES (%s, %s, %s, %s, %s, %s)", (voters_id, candidate_id, pres_election_id, polling_station_id, hmac_hash, ballot_id))
            db.execute_query("UPDATE voterinfo SET president_vote = %s WHERE voter_id = %s", (candidate_id, voters_id))
